...
# todo: dual normalizing flow network(DNF) to learn using distribution z to represent product image without background
...
class DNFNet(nn.Module):
    ...
    def forward(self, x, y):
        # x is the image input with background, y is image input without background
        output1 = self.nf1(x)
        output2 = self.nf2(y)
...
def nf_head_reverse(input_dim=c.n_feat):
    nodes = list()
    nodes.append(InputNode(input_dim, name='input'))
    for k in range(c.n_coupling_blocks):
        # nodes.
        # (Node([nodes[-1].out0], LUInvertibleMM, {'seed': k}, name=F'permute_{k}'))
        nodes.append(Node([nodes[-1].out0], glow_coupling_layer,
                          {'clamp': c.clamp_alpha, 'F_class': F_fully_connected,
                           'F_args': {'internal_size': c.fc_internal, 'dropout': c.dropout}},
                          name=F'fc_{k}'))
...
class DifferNet(nn.Module):
    def __init__(self):
        ...
        self.nf = nf_head()
        self.nf_reverse = nf_head_reverse()
